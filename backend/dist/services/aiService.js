 ...
 */
export async function askInitialQuestion(role, focusAreas, resumeContext, durationMinutes) {
 ...
 */
export async function askNextQuestion(history, role, focusAreas, resumeContext, durationMinutes) {
...
function extractTranscript(history) {
...
function buildRoleKeywords(role, focusAreas) {
    const lowerRole = role.toLowerCase();
    if (lowerRole.includes('frontend'))
        return TECH_KEYWORDS_BY_ROLE.frontend;
    if (lowerRole.includes('backend'))
        return TECH_KEYWORDS_BY_ROLE.backend;
    if (lowerRole.includes('hr') || lowerRole.includes('recruit') || lowerRole.includes('talent'))
        return TECH_KEYWORDS_BY_ROLE.hr;
    const focusKeywords = focusAreas
        .map((item) => item.toLowerCase())
        .filter(Boolean);
    return [...TECH_KEYWORDS_BY_ROLE.generic, ...focusKeywords];
}
function analyzeTranscript(history, role, focusAreas) {
    ...
    const communicationScore = clampScore(42 + averageWords * 0.55 + communicationCount * 5 + detailedAnswers * 4 - shortAnswers * 5 - uncertaintyCount * 3);
    const confidenceScore = clampScore(48 + leadershipCount * 3 + detailedAnswers * 3 - uncertaintyCount * 7 - shortAnswers * 2);
    const leadershipScore = clampScore(56 + leadershipCount * 5 + detailedAnswers * 2 - uncertaintyCount * 2);
    const technicalScore = clampScore(44 + technicalCount * 4 + focusAreas.length * 2 + averageWords * 0.15);
    const problemSolvingScore = clampScore(45 + problemSolvingCount * 5 + technicalCount * 1.5 + communicationCount * 1.5 - uncertaintyCount * 2);
    return {
        transcript,
        candidateAnswers,
        averageWords,
...
function buildRecommendedTopics(role, focusAreas, scores) {
    const topics = [];
    const lowerRole = role.toLowerCase();
...
function buildJustification(label, score, evidence, reduction) {
    return `${label}: ${score}. Evidence: ${evidence} Reduction: ${reduction}.`;
}
function buildFallbackEvaluation(history, role, focusAreas) {
    ...
    const overallScore = clampScore((radarChart.communication + radarChart.confidence + radarChart.leadership + radarChart.technical + radarChart.problemSolving) / 5);
    ...
    const scoreJustification = {
        communication: buildJustification('Communication score', radarChart.communication, `${analysis.communicationCount} structure markers were present across ${candidateCount} candidate answers, with an average answer length of ${analysis.averageWords.toFixed(1)} words.`, `${analysis.shortAnswers} short answers and ${analysis.uncertaintyCount} uncertainty phrases lowered clarity and completeness.`),
        confidence: buildJustification('Confidence score', radarChart.confidence, `${analysis.leadershipCount} ownership and action-oriented markers appeared in the transcript.`, `${analysis.uncertaintyCount} uncertainty phrases${uncertaintyExample ? ` including "${uncertaintyExample}"` : ''} reduced decisiveness.`),
        leadership: buildJustification('Leadership score', radarChart.leadership, `${analysis.leadershipCount} leadership or collaboration signals were detected, including ownership language where the candidate described work.`, 'A limited number of explicit leadership stories kept the score at a baseline rather than a top-band result.'),
        technical: buildJustification('Technical score', radarChart.technical, `${analysis.technicalCount} role-relevant technical terms matched the selected role and focus areas.`, 'The score was capped because the transcript did not consistently show deep implementation detail on every answer.'),
        problemSolving: buildJustification('Problem solving score', radarChart.problemSolving, `${analysis.problemSolvingCount} reasoning markers such as debugging, trade-off, and optimization language were observed.`, 'More explicit step-by-step reasoning would make the approach easier to evaluate.'),
    };
...
function normalizeEvaluation(raw, fallback) {
    ...
    const radarSource = source.radarChart && typeof source.radarChart === 'object' ? source.radarChart : {};
    const radarChart = {
    ...
    const scoreJustificationSource = source.scoreJustification && typeof source.scoreJustification === 'object'
        ? source.scoreJustification
        : {};
    ...
    return {
        ...
        recommendedTopics,
        scoreJustification: {
            communication: typeof scoreJustificationSource.communication === 'string' && scoreJustificationSource.communication.trim()
                ? scoreJustificationSource.communication
                : fallback.scoreJustification.communication,
            confidence: typeof scoreJustificationSource.confidence === 'string' && scoreJustificationSource.confidence.trim()
                ? scoreJustificationSource.confidence
                : fallback.scoreJustification.confidence,
            leadership: typeof scoreJustificationSource.leadership === 'string' && scoreJustificationSource.leadership.trim()
                ? scoreJustificationSource.leadership
                : fallback.scoreJustification.leadership,
            technical: typeof scoreJustificationSource.technical === 'string' && scoreJustificationSource.technical.trim()
                ? scoreJustificationSource.technical
                : fallback.scoreJustification.technical,
            problemSolving: typeof scoreJustificationSource.problemSolving === 'string' && scoreJustificationSource.problemSolving.trim()
